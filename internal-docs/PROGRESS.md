...
**Last updated:** 2026-04-02 (compiler + waves **7–9** baselines reflected in **`docs/`** + **README**)
...
| **✅ Baseline** | **7–9**   | **Compiler + content:** **`fromContent`**, **`fromMdxString`**, **`@better-seo/compiler`**: **`fromMdx`**, CLI **`content from-mdx`**. **CLI:** **`snapshot`**, **`preview`** (+ **`--open`**), **`init`**, **`template`**, TUI launcher, **`doctor`**, **`migrate`** hints. _Stretch (still 🟨 vs full PRD):_ MDX AST pipeline, **`analyze`** (Wave **10**), multi-network preview matrix (**L6**), template catalog depth (**L9–L11**). |
| **🟨 Partial**  | **10–12** | **10:** **`analyze`** shipped; **`add` / `scan` / `fix`** TBD. **11:** **`ogPaletteFromTokens`**; **`splash`** TBD. **12:** **`@better-seo/crawl`** + CLI syndication; hreflang / codemods TBD.                                                                                                                                                                                                                                           |
...
Waves **1–6** are **✅ done** (detail below). Waves **7–9** meet the **✅ baseline** row above (compiler + CLI + TUI surfaces); waves **10–12** and PRD stretch items (**scan/fix**, **`splash`**, **codemods**, full **L6**/**L9–L11**) remain **🟨** — see the partial table and [`Roadmap.md`](./Roadmap.md).
...
| **`@better-seo/compiler`**   | Partial (C17)     | **`fromMdx`** (**gray-matter** + core **`fromContent`**); CLI **`content from-mdx`**; not full MDX AST.                                                                                                                                                                                           |
