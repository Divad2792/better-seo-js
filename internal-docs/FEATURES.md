...
| C16 | **`fromContent`** (string → partial SEO, no extra parsers in core)                                           | `packages/core/src/from-content.ts`                                    | Optional; **`fromMdxString`** alias same path                    |
| C17 | **`fromMdx`** (frontmatter + body → partial SEO)                                                             | `packages/better-seo-compiler` (**`gray-matter`** + **`fromContent`**) | Not in zero-dep core; see [`ARCHITECTURE.md`](./ARCHITECTURE.md) |
